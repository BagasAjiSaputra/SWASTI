        ...
        <section className="py-12 px-6 sm:px-12 max-w-7xl mx-auto w-full">
            <div className="text-center mb-16 flex flex-col items-center">
                <motion.div
                    ...
                    viewport={{ once: true }}
                    className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-black mb-6 uppercase tracking-[0.2em]"
                >
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {stats.map((stat, i) => (
                        <Card key={stat.title} delay={0.1 * i} className="group hover:border-primary/50 transition-all duration-500">
                            <div className="flex justify-between items-start mb-8">
                                <div className="space-y-2">
                                    <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest opacity-80">{stat.title}</p>
                                    <div className="flex items-center gap-3">
                                        <h3 className="text-4xl font-black text-white tracking-tighter">{stat.value}</h3>
                                        <div className={cn(
                                            "flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full",
                                            stat.isUp ? "bg-red-500/10 text-red-500 border border-red-500/20" : "bg-green-500/10 text-green-500 border border-green-500/20"
                                        )}>
                                            {stat.isUp ? <TrendingUp size={10} className="mr-1" /> : <TrendingDown size={10} className="mr-1" />}
                                            {stat.change}
                                        </div>
                                    </div>
                                    <p className="text-xs font-medium text-muted-foreground italic flex items-center gap-1">
                                        <span className="w-1 h-1 rounded-full bg-primary/40 block" /> {stat.label}
                                    </p>
                                </div>
                                <div className={cn(
                                    ...
                                    stat.color === '#ef4444' ? "bg-red-500/10 text-red-500 border border-red-500/20" : stat.color === '#10b981' ? "bg-green-500/10 text-green-500 border border-green-500/20" : "bg-blue-500/10 text-blue-500 border border-blue-500/20"
                                )}>
                                    {stat.color === '#ef4444' ? <AlertCircle size={24} /> : stat.color === '#10b981' ? <TrendingUp size={24} /> : <TrendingDown size={24} />}
                                </div>
                            </div>

                            <div className="h-28 w-full -mb-6 -mx-4 opacity-60 group-hover:opacity-100 transition-all duration-700">
                                <Sparkline data={stat.data} width={400} height={112} color={stat.color} />
                            </div>
                        </Card>
                    ))}
                </div>
